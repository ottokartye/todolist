todoListApp.controller("TodoListController", function ($scope, LocalStorageFactory, CalendarFactory) {
    ...
    $scope.getTasksCompletionPercentage = function (projectId) {
        var completedTasks = $scope.getCompletedTasksCount(projectId);
        var totalTasks = 0;
        if (projectId == undefined) {
            totalTasks = $scope.tasks.length;
        } else {
            totalTasks = $scope.getTasksForProjectId(projectId);
        }
        $scope.completionPercentage = completedTasks * 100 / totalTasks;
    ...
    $scope.openTaskWizard = function (taskId = null) {
        if (taskId != null) {
            $.each($scope.tasks, function (index) {
    ...
    $scope.getTasksForProjectId = function (projectId) {
        var numberOfTasks = 0;
        $.each($scope.tasks, function (index) {
            if ($scope.tasks[index].project_id === projectId) numberOfTasks += 1;
        });
    ...
    $scope.getTasksByStatus = function (status = null) {
        var foundTasks = [];
        $.each($scope.tasks, function (index) {
            if ($scope.filterByProjectId.length != 0) {
                var i = $scope.filterByProjectId.indexOf($scope.tasks[index].project_id);
